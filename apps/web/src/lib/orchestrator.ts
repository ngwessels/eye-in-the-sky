    ...
    for (const { station, meta } of top) {
      ...
      await enqueueCommand({
        ...
        selection_reason: station.capabilities?.omni_quad
          ? `omni_quad score=${meta.score.toFixed(3)} dist_km=${meta.distance.toFixed(1)} precip=${precip.toFixed(2)}`
          : `score=${meta.score.toFixed(3)} dist_km=${meta.distance.toFixed(1)} precip=${precip.toFixed(2)}`,
      });
      enqueued += 1;
    }
  }
